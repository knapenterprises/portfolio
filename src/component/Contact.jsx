...
const Contact = () => {
  ...
  const onHandleCheck = () => {
    ...
    if (!hasError) {
      const subject = "Hello From ".concat(data.name).replaceAll(" ", "%20");
      const body = data.message.replaceAll(" ", "%20");
      const link = `mailto:info@knapenterprises.in?subject=${subject}&body=${body}`;
      ...
      >
        ...
        >
          ...
          <Box py={2}>
            ...
            />
            <Typography
              sx={{ color: "red", fontSize: "15px", fontWeight: "700" }}
            >
              {data.error.name}
            </Typography>
          </Box>
          ...
          >
            <button className="boton-elegante" onClick={onHandleCheck}>
              Explore
            </button>
          </Stack>
        </Box>
